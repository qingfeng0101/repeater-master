...
import { credentialWecomList, credentialWecomDelete } from "@/api/stmp"
...
const columns: DataTableColumn<DataSource>[] = [
  { title: '名称', key: 'name' },
  { title: '应用ID', key: 'agent_id' },
  { title: '企业ID', key: 'corp_id' },
  { title: '凭证密钥', key: 'corp_secret' },
...
function handleClick() {
  router.push({ name: 'CredentialWecomAdd' })
}



function handleDelete(row: DataSource) {
  credentialWecomDelete(row.id)
    .then(() => {
